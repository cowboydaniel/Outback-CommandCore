...
from PySide6.QtWidgets import (
    ...
    QDialogButtonBox, QApplication, QMessageBox, QGraphicsView,
    QGraphicsScene
)
from PySide6.QtGui import QFont, QIcon, QPainterPath, QPen, QBrush, QColor, QPainter
        ...
        try:
            ...
            for dev in os.listdir('/sys/block'):
                ...
                if any(dev.startswith(x) for x in skip_prefixes):
                    print(f"  - Skipping (virtual/ignored device type)")
                    continue
...
class ResourceGraph(QGraphicsView):
    ...
    def update_graph(self):
        ...
        self.scene.addRect(plot_area, QPen(QColor(100, 100, 120), 1), QBrush(QColor(45, 45, 55)))
        
        # Draw grid lines - fixed Y-axis from 0-100%
        pen = QPen(self.grid_color, 1, Qt.PenStyle.DotLine)
        
        # Horizontal grid lines (percentage)
        for percent in range(0, 101, 20):  # 0% to 100% in 20% increments
            y = height - (percent / 100 * height)
            self.scene.addLine(self.margins['left'], y, width + self.margins['left'], y, pen)
            
            # Add percentage labels
            if percent > 0:  # Don't show 0% label at the very bottom
                label = self.scene.addText(f"{percent}%", QFont("Arial", 8))
                label.setPos(self.margins['left'] + 5, y - 10)
                label.setDefaultTextColor(Qt.GlobalColor.lightGray)
        
        # Vertical grid lines (time)
        current_time = time.time()
        for minutes in range(0, 11, 1):  # Every minute for 10 minutes
            if minutes == 0:
                continue  # Skip 0 to avoid overlapping with Y-axis
                
            # Calculate x position from the right (most recent data)
            time_ago = minutes * 60  # Convert minutes to seconds
            x = self.margins['left'] + (width * (1 - (time_ago / 600)))  # 600 seconds = 10 minutes
            
            if x >= 0:  # Only draw if within visible range
                self.scene.addLine(x, 0, x, height, pen)
                
                # Add time labels (only show every 2 minutes to avoid clutter)
                if minutes % 2 == 0:  # Only show even minutes (0, 2, 4, 6, 8, 10)
                    label = self.scene.addText(f"-{minutes}m", QFont("Arial", 8))
                    label.setPos(x - 10, height + self.margins['top'])
                    label.setDefaultTextColor(Qt.GlobalColor.lightGray)
        
        # Always use 10-minute scale, even with less data
        ten_minutes_seconds = 600
        current_time = time.time()
        
        # Draw the graph if we have data points
        if self.data_points:
            path = QPainterPath()
            
            # Calculate graph boundaries
            left_edge = self.margins['left']
            right_edge = self.margins['left'] + width
            top_edge = self.margins['top']
            bottom_edge = self.margins['top'] + height
            
            # Add first point (ensure it's within bounds)
            first_value = self.data_points[0]
            first_time = self.time_data[0]
            time_ago = current_time - first_time
            x = max(left_edge, min(right_edge, left_edge + (width * (1 - (time_ago / ten_minutes_seconds)))))
            y = max(top_edge, min(bottom_edge, top_edge + (height - (first_value / 100 * height))))
            path.moveTo(x, y)
            
            # Add remaining points (newest to oldest, right to left)
            prev_x = x
            prev_y = y
            
            for value, timestamp in zip(self.data_points[1:], self.time_data[1:]):
                time_ago = current_time - timestamp
                x = left_edge + (width * (1 - (time_ago / ten_minutes_seconds)))
                y = top_edge + (height - (value / 100 * height))
                
                # Only add point if it's within the visible area
                if left_edge <= x <= right_edge and top_edge <= y <= bottom_edge:
                    path.lineTo(x, y)
                elif x < left_edge and prev_x >= left_edge:
                    # Calculate intersection with left edge
                    if prev_x != x:  # Avoid division by zero
                        t = (left_edge - x) / (prev_x - x)
                        y_intersect = y + t * (prev_y - y)
                        path.lineTo(left_edge, max(top_edge, min(bottom_edge, y_intersect)))
                
                prev_x = x
                prev_y = y
            
            # Draw the line
            line_pen = QPen(self.line_color, 1.5)
            self.scene.addPath(path, line_pen)
        
        # Draw current value and time in top-right corner
        current_time_str = time.strftime("%H:%M:%S")
        text = f"{self.current_value:.1f}% @ {current_time_str}"
        text_item = self.scene.addText(text, QFont("Arial", 10, QFont.Weight.Bold))
        text_rect = text_item.boundingRect()
        text_item.setPos(
            self.margins['left'] + width - text_rect.width() - 10, 
            self.margins['top'] - 30  # Position above the graph
        )
        text_item.setDefaultTextColor(Qt.GlobalColor.white)  # White text for better visibility
        
        # Draw title
        title = self.scene.addText("CPU Usage" if hasattr(self, 'is_cpu') and self.is_cpu else "Memory Usage", 
                                 QFont("Arial", 12, QFont.Weight.Bold))
        title.setPos(self.margins['left'], 10)
        title.setDefaultTextColor(Qt.GlobalColor.white)  # White title text