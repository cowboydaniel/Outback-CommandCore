...
class FileDialog:
    ...
    def askdirectory(title=None, initialdir=None):
        return QtWidgets.QFileDialog.getExistingDirectory(None, title or "Select Directory", initialdir or "")
...
class ttk:
    ...
    Treeview = Treeview
            ...
            screenshot_window.setWindowTitle(f"Android Screenshot - {os.path.basename(screenshot_path)}")
            layout = QtWidgets.QVBoxLayout(screenshot_window)

            pixmap = QtGui.QPixmap(screenshot_path)
            image_label = QtWidgets.QLabel()
            image_label.setPixmap(pixmap)
            image_label.setAlignment(QtCore.Qt.AlignLeft | QtCore.Qt.AlignTop)
            ...
            screenshot_window.resize(min(pixmap.width(), 1000), min(pixmap.height(), 800))
            screenshot_window.exec()
                        self.log_message(
                            f"Raw CPU stats: First line of /proc/stat: {output.splitlines()[0]}"
                        )
